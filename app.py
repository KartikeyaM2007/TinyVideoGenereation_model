...
def generate_2frame(class_name: str, steps: int, guidance: float):
    ...
    gif_path = Path("outputs/video2f_samples") / f"{class_name}_sample_1.gif"
    side_path = Path("outputs/video2f_samples") / f"{class_name}_sample_1_sidebyside.png"
...
def main():
    st.set_page_config(
        page_title="AnomVidGen",
        page_icon="ðŸŽ¥",
        layout="wide",
    )

    st.title("ðŸŽ¥ AnomVidGen")
    st.caption(
    ...
    with st.sidebar:
        ...
        mode = st.radio(
            "Mode",
            ["Image generation", "2 frame GIF generation"],
        )

        input_type = st.radio(
            "Input type",
            ["Class selector", "Text prompt"],
        )
        ...
        if input_type == "Class selector":
            ...
            prompt = st.text_input(
                "Prompt",
                value="a road accident surveillance scene",
            )
    ...
    st.code(
        "class/text input â†’ class conditioned diffusion â†’ surveillance image or 2 frame GIF",
        language="text",
    )
